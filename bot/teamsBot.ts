...
import { BearerAuthProvider, createApiClient, IdentityType, TeamsFx } from "@microsoft/teamsfx";
  ...
  constructor() {
    ...
    this.onMessage(async (context, next) => {
      ...
      switch (txt) {
        ...
        case "func": {
          await context.sendActivity(`Calling function!`);

          const teamsfx = new TeamsFx(IdentityType.App);
          const appCredential = teamsfx.getCredential();
          const functionEndpoint = "http://localhost:7071";
          const teamsAuthAADAppId = "5195ff57-e568-43fa-a8a8-6802d6b9c870";
          const defaultScope = `${teamsAuthAADAppId}/.default`;
          const bearerTokenAuthProvider = new BearerAuthProvider(async () => (await appCredential.getToken(defaultScope))?.token || "");

          // Create API client
          const functionClient = createApiClient(functionEndpoint, bearerTokenAuthProvider);
          const functionName = "getUserProfile";
          const response = await functionClient.get("/api/"+functionName);
